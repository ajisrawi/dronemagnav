# DroneMagNav â€” Design Specification (rev A)
...
| Sensing | LIS3MDL mag, BMI088 IMU, BMP280 baro â€” one SPI bus at 8â€“10 MHz |
| Reference | u-blox MAX-M10S GNSS on UART0 pads (GPIO43/44), PPS on GPIO16 |
| Map storage | microSD, SDMMC 4-bit (GPIO38â€“42, 47), card detect GPIO48 |
...
| Power | USB-C VBUS âŠ• FC 5 V (SS14 diode-OR) â†’ AMS1117-3.3 â†’ +3V3; AP2112K-3.3 â†’ +3.3VA |
...
## 4. Connectors
- **J1 USB-C** â€” power (VBUS) + native USB (D+/Dâˆ’ through USBLC6-2SC6 ESD).
- **J2 FC_TELEM (JST-GH 6)** â€” 1 VCC 5 V in, 2 FC TX â†’ board, 3 board TX â†’ FC, 4/5 NC, 6 GND.
- **J3 microSD** â€” Hirose DM3AT push-push.
- **J4 GNSS antenna** â€” U.FL, passive antenna, 50 Î© trace.
- **J5 SENSOR_MAST (JST-GH 6)** â€” 1 +3.3VA, 2 GND, 3 SDA, 4 SCL, 5 INT, 6 +5 V.

## 5. PCB
64 Ã— 60 mm, 4 layers (F.Cu signals / In1 GND / In2 +3V3 / B.Cu signals),
...
- **AnomalyMap** 96 Ã— 96-cell PSRAM window over the int16 nT grid on SD; bilinear lookup.
...
`/maps/wdmam.bin` (32-byte header + int16 nT, 3â€² world grid â‰ˆ 52 MB), and
stages `WMM.COF`, `coast.json` (Natural Earth 1:110m) and the web page.

## 8. Verification
| Check | Result |
|---|---|
| Schematic ERC | 0 errors, 1 warning (BMI088 SDO1/SDO2 both on MISO â€” per Bosch reference design) |
...
## 9. Known limitations
- WDMAM resolution/altitude â†’ km-class accuracy without a local survey.
