# DroneMagNav â€” magnetic-anomaly navigation board for drones

GNSS-denied positioning from the Earth's crustal magnetic field, plus a
phone-operated great-circle route planner.

```
  LIS3MDL magnetometer â”€â”                          â”Œâ”€ WDMAM anomaly map (SD)
  BMI088 IMU â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¼â”€ ESP32-S3 â”€ particle â”€â”€â”€â”€â”¤  WMM core field model
  BMP280 baro â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜   filter    matching     â””â”€ Tolles-Lawson compensation
        â”‚                        â”‚
   u-blox GNSS (init/truth)      â”œâ”€ MAVLink GPS_INPUT â†’ flight controller
                                 â””â”€ WiFi AP â†’ phone: offline map, route, upload
```
...
| PCB 64 Ã— 60 mm, 4-layer, 30.5 mm Pixhawk hole pattern | `hardware/dronemagnav/dronemagnav.kicad_pcb` | routed, 0 unconnected, 0 electrical DRC |
...
| SD-card preparation | `tools/prepare_sd.py` | WDMAM â†’ binary grid, WMM, coastline |
...
| Power | USB/FC 5 V â†’ AMS1117-3.3 (digital) + AP2112K-3.3 (sensors) | separate clean sensor rail |
...
3. **Update** each particle against *WMM core field + WDMAM anomaly* at the
   particle's location; resample; output mean and 1-Ïƒ.
...
*kilometre-class* positioning â€” right for oceanic/jammed cruise, not landing.
